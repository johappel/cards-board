...
Dieses Dokument gibt eine Übersicht über das Projekt. Für detaillierte Konzepte siehe [[KONZEPT|KONZEPT.md]] und [[Codebase-Analyse|analyse.md]]. Nostr-Integration in [[Nostr Integration|NOSTR-INTEGRATION.md]]. Wichtige Regeln in [[AGENTS|AGENTS.md]]. Siehe [[INDEX|INDEX.md]] für Navigation.

## Inhaltsverzeichnis
- [[1. Vision und Projektziel|#1-vision-und-projektziel]]
- [[2. Kernphilosophie und Architektur|#2-kernphilosophie-und-architektur]]
- [[3. Implementierte Features|#3-implementierte-features]]
- [[4. Projektstatus und Ausblick|#4-projektstatus-und-ausblick]]
...
- **Modularität:** Der Code ist klar in Funktionsmodule gegliedert (`storage.js`, `board.js`, `card.js`, `drag_drop.js`, `import_export.js`, `share_via_nostr.js`, `onpaste.js`, `ai.js`), was die Wartbarkeit und Erweiterbarkeit verbessert. Details in [[Codebase-Analyse|analyse.md]].
...
- **WYSIWYG-Editor:** Ein in das Karten-Modal integrierter Rich-Text-Editor (basierend auf Quill.js) erleichtert die Inhaltserstellung. Siehe [[Quill Save-Only Implementation|QUILL-SAVE-ONLY-IMPLEMENTATION.md]].
...
- **Frei konfigurierbare Endpoints:** Der Benutzer kann in den Einstellungen eigene Endpoints für KI-Dienste oder Automatisierungsworkflows hinterlegen. Dies unterstützt sowohl Standard-LLM-Provider als auch benutzerdefinierte Systeme wie **n8n**. Siehe [[AI-Supported Endpoints|AI-SUPPORTED-ENDPOINTS.md]].
...
- **Nostr-Integration:**
  ...
  - **Einfacher Import:** Über einen `nevent`-Link kann jeder ein geteiltes Board mit einem Klick importieren, ohne dass eine zentrale Instanz involviert ist. Siehe [[Nostr Implementation Complete|NOSTR-IMPLEMENTATION-COMPLETE.md]].

### 3.4. Erweiterte Card-Features
- **Labels:** Karten können mit Tags versehen werden, siehe [[Labels Implementation|LABELS-IMPLEMENTATION.md]].
- **Kommentare & URLs:** Unterstützung für Kommentare und klickbare Links in Karten, siehe [[Kommentare & URLs Implementation|IMPLEMENTATION-COMMENTS-URLS.md]].
- **Summary:** Automatische Board-Zusammenfassungen, siehe [[Board Summary Implementation|BOARD-SUMMARY-IMPLEMENTATION.md]] und [[Summary Implementation Status|SUMMARY-IMPLEMENTATION-STATUS.md]].
...
---
*Überarbeitet für Wiki: Hinzugefügtes Inhaltsverzeichnis, Links zu allen relevanten Docs, erweiterte Struktur mit Unterabschnitten.*